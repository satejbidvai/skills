...
Spawn all explorers in parallel. Each explorer gets the same base prompt from `references/explorer-prompt.md` plus a specific exploration angle naming its slice. Each explorer should:
...
Spawn a single subagent that explores and explains in one pass. The agent does its own exploration (Glob, Grep, Read) and writes the explanation directly. Read `references/explainer-prompt.md` for the communication style and output format. Same structure, just no explorer findings as input.
...
After the explanation is complete, spawn 2-3 architectural critics in parallel.
